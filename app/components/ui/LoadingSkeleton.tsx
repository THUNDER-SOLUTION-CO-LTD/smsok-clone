export function TableSkeleton({ rows = 5 }: { rows?: number }) {
  return (
    <div className="bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-[20px] overflow-hidden animate-fade-in">
  ...
  return (
    <div className={`grid grid-cols-2 sm:grid-cols-${count} gap-4 stagger-children`}>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-[20px] p-5">
...
export function CardSkeleton() {
  return (
    <div className="bg-[var(--bg-surface)] border border-[var(--border-default)] rounded-[20px] p-6 animate-fade-in">
