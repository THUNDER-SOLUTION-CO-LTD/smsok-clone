  ...
  return (
    <div className="p-4 md:p-6 pb-20 md:pb-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        {/* ── Compose Panel ── */}
        <Card className="lg:col-span-3 bg-[var(--bg-surface)] border-[var(--border-default)] rounded-[20px]">
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <Select value={senderName} onValueChange={(v) => v && setSenderName(v)}>
                  <SelectTrigger className="h-11 bg-[var(--bg-base)] border-[var(--border-default)] text-white rounded-xl focus:border-[rgba(0,255,167,0.6)] focus:ring-[rgba(0,255,167,0.15)]">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-[var(--bg-surface)] border-[var(--border-default)] rounded-xl">
                    {senderNames.map((name) => (
                      <SelectItem key={name} value={name} className="hover:bg-[var(--bg-base)] text-white">
                        {name}
                      </SelectItem>
                    ))}
                  </SelectContent>
                  ...
                  <TabsList className="h-9 bg-transparent gap-2 p-0">
                    {([
                      ...
                      <TabsTrigger
                        key={tab.value}
                        value={tab.value}
                        className="h-9 px-4 rounded-lg text-xs font-medium border border-[var(--border-default)] text-[var(--text-muted)] data-[state=active]:bg-[rgba(0,255,167,0.08)] data-[state=active]:border-[rgba(0,255,167,0.3)] data-[state=active]:text-[var(--accent)]"
                      >
                        {tab.label}
                      </TabsTrigger>
                    ))}
              ...
              <div>
                ...
                <Textarea
                  ...
                  className={`bg-[var(--bg-base)] border-[var(--border-default)] text-white placeholder:text-[var(--text-muted)] rounded-xl resize-none focus:border-[rgba(0,255,167,0.6)] focus:ring-[rgba(0,255,167,0.15)] ${phoneError ? "border-[rgba(239,68,68,0.6)]" : validPhones.length > 0 ? "border-[rgba(16,185,129,0.4)]" : ""}`}
                  rows={4}
              ...
              <div>
                ...
                <Textarea
                  ...
                  className="bg-[var(--bg-base)] border-[var(--border-default)] text-white placeholder:text-[var(--text-muted)] rounded-xl resize-none focus:border-[rgba(0,255,167,0.6)] focus:ring-[rgba(0,255,167,0.15)]"
                  rows={5}
          ...
          <Card className="bg-[var(--bg-surface)] border-[var(--border-default)] rounded-[20px]">
            <CardContent className="p-5">
              ...
              <div className="space-y-2.5">
                ...
                <div className="flex justify-between text-[13px] border-b border-[var(--border-default)] pb-2.5">
                  <span className="text-[var(--text-muted)]">เครดิต/SMS</span>
              ...
              <Button
                ...
                className="w-full mt-4 h-12 bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-[var(--bg-base)] rounded-[20px] font-semibold disabled:opacity-50"
              >
                {isPending ? (
      ...
      <div className="lg:hidden fixed bottom-16 left-0 right-0 z-40 bg-[var(--bg-base)] border-t border-[var(--border-default)] px-4 py-3 safe-area-bottom">
        <div className="flex items-center justify-between gap-4">
          ...
          <Button
            onClick={handleSend}
            disabled={isPending || !recipients.trim() || !message.trim() || !!phoneError}
            className="h-11 px-6 bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-[var(--bg-base)] rounded-[20px] font-semibold disabled:opacity-50"
          >
            {isPending ? (
