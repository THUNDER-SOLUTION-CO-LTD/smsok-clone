  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto animate-fade-in-up">
      <div className="glass p-12 md:p-16 text-center relative overflow-hidden">
        ...
        <div className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-gradient-to-br from-violet-500/10 to-cyan-500/10 border border-violet-500/10 flex items-center justify-center relative">
          <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" className="text-violet-400">
            <path d="M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z" />
          </svg>
        </div>

        <h2 className="text-2xl font-bold mb-3 tracking-tight relative">
          <span className="gradient-text-mixed">แคมเปญ SMS</span>
          <span className="ml-3 badge-glow-info">เร็วๆ นี้</span>
        </h2>

        <p className="text-sm text-[var(--text-secondary)] mb-6 max-w-md mx-auto relative">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8 max-w-lg mx-auto relative stagger-children">
          {[
            ...
            <div key={feature.label} className="glass p-3 rounded-xl text-center">
              ...
              <p className="text-[11px] text-[var(--text-muted)]">{feature.label}</p>
            </div>
          ))}
        </div>
