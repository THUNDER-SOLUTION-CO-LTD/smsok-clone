...
import { addContactToGroup, removeContactFromGroup, bulkRemoveFromGroup } from "@/lib/actions/groups";
...
export default function GroupDetailClient({
  ...
  }, [availableContacts, memberContactIds, addSearch]);
  ...
  const toggleSelectAll = () => {
    if (allSelected) {
      ...
      setSelectedIds(new Set(paginatedMembers.map((m) => m.contactId)));
    }
  };
  ...
  const toggleAddSelectAll = () => {
    if (allAddSelected) {
      ...
      setAddSelectedIds(new Set(filteredAvailable.map((c) => c.id)));
    }
  };
    ...
    >
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <motion.button
              onClick={() => { setShowAddModal(true); setAddSearch(""); setAddSelectedIds(new Set()); }}
      ...
      {paginatedMembers.length > 0 ? (
            ...
            <div className="px-5 py-4 border-t border-[var(--border-subtle)] flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  disabled={page <= 1}
                  className="px-3 py-1.5 text-xs rounded-lg border border-[var(--border-subtle)] text-[var(--text-muted)] hover:text-white hover:border-violet-500 disabled:opacity-30 transition-colors"
                >
                  ← ก่อนหน้า
                </button>
        ...
        <motion.div className="glass p-16 text-center" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
          ...
          <button
            onClick={() => { setShowAddModal(true); setAddSearch(""); setAddSelectedIds(new Set()); }}
            className="btn-primary px-6 py-2.5 rounded-xl text-sm inline-flex items-center gap-2 cursor-pointer"
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><line x1="12" y1="5" x2="12" y2="19" /><line x1="5" y1="12" x2="19" y2="12" /></svg>
            เพิ่มรายชื่อ
          </button>
      ...
      <AnimatePresence>
        {showAddModal && (
          <motion.div className="fixed inset-0 z-50 flex items-center justify-center px-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <motion.div
              className="relative glass w-full max-w-lg z-10 flex flex-col max-h-[80vh]"
            ...
            >
                ...
                {filteredAvailable.length > 0 && (
                  <div className="flex items-center justify-between mt-3">
                    ...
                    {addSelectedIds.size > 0 && (
                      <span className="text-xs text-violet-400">{addSelectedIds.size} เลือกแล้ว</span>
                    )}
                  </div>
                )}
              </div>
              ...
              <div className="flex-1 overflow-y-auto px-6 pb-4 space-y-1">
                {filteredAvailable.length === 0 ? (
                  <div className="text-center py-8 text-xs text-[var(--text-muted)]">
                    {availableContacts.length === 0 ? "ไม่มีรายชื่อที่สามารถเพิ่มได้" : "ไม่พบที่ค้นหา"}
                  </div>
                ) : (
              ...
              <div className="px-6 py-4 border-t border-[var(--border-subtle)] flex gap-3">
                ...
                >
                  {isPending ? "กำลังเพิ่ม..." : `เพิ่ม ${addSelectedIds.size > 0 ? addSelectedIds.size + " รายชื่อ" : "ทั้งหมด"}`}
                </button>
              </div>
            </motion.div>
          </motion.div>
