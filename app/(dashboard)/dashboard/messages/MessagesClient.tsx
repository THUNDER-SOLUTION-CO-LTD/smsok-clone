      ...
      <div className="space-y-3">
        ...
        <div className="hidden sm:flex flex-wrap items-end gap-3">
          <div className="flex flex-col gap-1">
            ...
            <Input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} onClick={(e) => { setDateFromFocused(true); try { (e.currentTarget as HTMLInputElement).showPicker(); } catch {} }} onFocus={() => setDateFromFocused(true)} onBlur={() => setDateFromFocused(false)} style={{ colorScheme: "dark" }} className={`h-9 text-sm w-[150px] bg-[var(--bg-base)] text-[var(--text-primary)] rounded-lg cursor-pointer transition-colors border ${dateFromFocused ? "border-ring ring-3 ring-ring/50" : "border-[var(--border-default)]"}`} />
          </div>
          <div className="flex flex-col gap-1">
            <span className="text-[10px] font-semibold uppercase tracking-wider text-[var(--text-muted)] px-1">ถึง</span>
            <Input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} onClick={(e) => { setDateToFocused(true); try { (e.currentTarget as HTMLInputElement).showPicker(); } catch {} }} onFocus={() => setDateToFocused(true)} onBlur={() => setDateToFocused(false)} style={{ colorScheme: "dark" }} className={`h-9 text-sm w-[150px] bg-[var(--bg-base)] text-[var(--text-primary)] rounded-lg cursor-pointer transition-colors border ${dateToFocused ? "border-ring ring-3 ring-ring/50" : "border-[var(--border-default)]"}`} />
          </div>
          {hasFilters && (
            <Button
              variant="ghost"
